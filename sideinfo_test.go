...
func TestReadSideInfo_Stereo_WindowSwitching(t *testing.T) {
    ...
    // For each granule/channel: choose window_switching = 1, block_type = 2, mixed_block_flag = 0
    for gr := 0; gr < 2; gr++ {
        for ch := 0; ch < 2; ch++ {
            ...
            bw.write(2, 2)      // block_type = 2 (short blocks)
            ...
            // Region counts are derived for block_type=2 && !mixed_block
    ...
    for gr := 0; gr < 2; gr++ {
        for ch := 0; ch < 2; ch++ {
            ...
            if g.GetBlockType() != 2 {
                t.Fatalf("granule[%d][%d] block type got %d, want 2", gr, ch, g.GetBlockType())
            }
...
func TestReadSideInfo_WindowSwitchingInvalidBlockType(t *testing.T) {
    // Mono, no CRC to minimize size; set invalid combination: window_switching=1 with block_type=0
    var h MP3FrameHeader
    h.flag1 |= 1 << 6 // no CRC
    ...
    for gr := 0; gr < 2; gr++ {
        ...
        bw.write(4, 0x1)
        bw.write(1, 1) // window_switching = 1
        bw.write(2, 0) // block_type = 0 (invalid with window_switching)
    ...
    if err == nil {
        t.Fatalf("expected error for invalid block_type with window_switching, got nil")
    }
}
